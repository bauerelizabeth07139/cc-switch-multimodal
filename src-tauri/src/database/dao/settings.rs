...
impl Database {
    ...
    /// 获取多模态路由配置
    pub fn get_multimodal_routing_config(
        &self,
    ) -> Result<crate::proxy::types::MultimodalRoutingConfig, AppError> {
        match self.get_setting("multimodal_routing_config")? {
            Some(json) => serde_json::from_str(&json).map_err(|e| {
                AppError::Database(format!("解析多模态路由配置失败: {e}"))
            }),
            None => Ok(crate::proxy::types::MultimodalRoutingConfig::default()),
        }
    }

    /// 更新多模态路由配置
    pub fn set_multimodal_routing_config(
        &self,
        config: &crate::proxy::types::MultimodalRoutingConfig,
    ) -> Result<(), AppError> {
        let json = serde_json::to_string(config).map_err(|e| {
            AppError::Database(format!("序列化多模态路由配置失败: {e}"))
        })?;
        self.set_setting("multimodal_routing_config", &json)
    }
