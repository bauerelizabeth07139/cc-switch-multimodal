...
/// 获取多模态路由配置
#[tauri::command]
pub async fn get_multimodal_routing_config(
    state: tauri::State<'_, crate::AppState>,
) -> Result<crate::proxy::types::MultimodalRoutingConfig, String> {
    state
        .db
        .get_multimodal_routing_config()
        .map_err(|e| e.to_string())
}

/// 设置多模态路由配置
#[tauri::command]
pub async fn set_multimodal_routing_config(
    state: tauri::State<'_, crate::AppState>,
    config: crate::proxy::types::MultimodalRoutingConfig,
) -> Result<bool, String> {
    state
        .db
        .set_multimodal_routing_config(&config)
        .map_err(|e| e.to_string())?;
    Ok(true)
}
...
pub async fn list_composite_models(
    state: tauri::State<'_, crate::AppState>,
) -> Result<Vec<crate::proxy::types::CompositeModelBinding>, String> {
    let config = state
        .db
        .get_multimodal_routing_config()
        .map_err(|e| e.to_string())?;
    Ok(config.composite_bindings)
}

/// 添加复合模型绑定
#[tauri::command]
pub async fn add_composite_model(
    state: tauri::State<'_, crate::AppState>,
    binding: crate::proxy::types::CompositeModelBinding,
) -> Result<bool, String> {
    let mut config = state
        .db
        .get_multimodal_routing_config()
        .map_err(|e| e.to_string())?;
    config.composite_bindings.push(binding);
    state
        .db
        .set_multimodal_routing_config(&config)
        .map_err(|e| e.to_string())?;
...
/// 按名称移除复合模型绑定
#[tauri::command]
pub async fn remove_composite_model(
    state: tauri::State<'_, crate::AppState>,
    name: String,
) -> Result<bool, String> {
    let mut config = state
        ...
        .get_multimodal_routing_config()
        .map_err(|e| e.to_string())?;
    let initial_len = config.composite_bindings.len();
    config
        .composite_bindings
        .retain(|b| b.name != name);
    if config.composite_bindings.len() == initial_len {
        return Err(format!("未找到名称为 '{name}' 的复合模型绑定"));
    }
    state
        .db
        .set_multimodal_routing_config(&config)
