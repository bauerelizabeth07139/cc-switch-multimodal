...
use super::{
    ...
    multimodal_router::execute_eyes_inference,
    ...
    types::{CopilotOptimizerConfig, MultimodalRoutingConfig, OptimizerConfig, ProxyStatus, RectifierConfig},
    ProxyError,
};
...
pub struct RequestForwarder {
    ...
    /// 多模态路由配置
    multimodal_config: MultimodalRoutingConfig,
    /// 非流式请求超时（秒）
...
impl RequestForwarder {
    ...
    fn apply_multimodal_auto_route(&self, body: &mut Value, provider: &Provider) -> bool {
        ...
        if capability != ImageInputCapability::Unsupported {
            return false;
        }
        ...
        log::info!(
            "[Multimodal] Auto-routed text-only model '{}' to multimodal model '{}' for provider={}",
            model,
            fallback_model,
            provider.id
        );
    ...
    ///
    /// 如果多模态自动路由已处理了图片，则不再 stripping。
    fn apply_media_prevention(&self, body: &mut Value, provider: &Provider) -> usize {
        // 多模态自动路由优先：如果已将 text-only 模型路由到多模态模型，不 stripping
        if self.apply_multimodal_auto_route(body, provider) {
            return 0;
        }
    ...
    async fn forward_with_retry_inner(
        ...
        if self.multimodal_config.enabled {
            ...
            if let Some(binding) = self
                ...
                .composite_bindings
                .iter()
                .find(|b| b.name == request_model)
            {
                log::info!(
                    "[Multimodal] Composite model '{}' matched, eyes={} brain={}",
                    binding.name,
                    binding.eyes_model,
                    binding.brain_model
                );
                ...
                // 执行 eyes 推理
                let mut eyes_succeeded = false;
                let eyes_provider_opt = match self
                    .router
                    .get_provider_by_id(&binding.eyes_provider_id, app_type_str)
                    .await
                {
                    Ok(p) => p,
                    Err(e) => {
                        log::warn!(
                            "[Multimodal] Failed to look up eyes provider '{}': {e}",
                ...
                if let Some(eyes_provider) = eyes_provider_opt {
                    if let Ok(eyes_base_url) = adapter.extract_base_url(&eyes_provider) {
                        match execute_eyes_inference(
                            &body,
                            &eyes_provider,
                            &binding.eyes_model,
                            app_type,
                        ...
                        .await
                        {
                            ...
                            Err(e) => {
                                log::warn!(
                                    "[Multimodal] Eyes inference failed: {e}, falling back to original request"
                                );
                            }
                        }
                    }
                ...
                // 仅在 eyes 推理成功时切换到大脑模型和供应商
                if eyes_succeeded {
                    let brain_provider_opt = match self
                        .router
                        .get_provider_by_id(&binding.brain_provider_id, app_type_str)
                        .await
                    {
                        Ok(p) => p,
                        Err(e) => {
                            log::warn!(
                                "[Multimodal] Failed to look up brain provider '{}': {e}",
                    ...
                    if let Some(brain_provider) = brain_provider_opt {
                        body["model"] = Value::String(binding.brain_model.clone());
                        providers = vec![brain_provider];
                        log::info!(
                            "[Multimodal] Eyes inference completed, switched to brain model '{}' and provider '{}'",
                            binding.brain_model,
                            binding.brain_provider_id
                        );
                    } else {
                        log::warn!(
                            "[Multimodal] Brain provider '{}' not found, keeping eyes result with original model '{}'",
                            binding.brain_provider_id,
                            request_model
                        );
                    }
